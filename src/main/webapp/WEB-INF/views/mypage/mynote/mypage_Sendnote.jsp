...
<%
	...
	for(NoteTO noteTO : listTO.getNoteList()) {
		...
		if(noteTO.getStatus() == 0) {
			getNoteHTML.append("<label class='form-check-label' onclick=\"location.href='/mailSendView?seq="+noteTO.getSeq()+"'\"><i class='bi bi-envelope'></i> 수신자 : "+noteTO.getReceiverSeq()+" | 제목 : "+noteTO.getSubject()+" </label>");
		} else {
			getNoteHTML.append("<label class='form-check-label' onclick=\"location.href='/mailSendView?seq="+noteTO.getSeq()+"'\"><i class=\"bi bi-envelope-open\"></i> 수신자 : "+noteTO.getReceiverSeq()+" | 제목 : "+noteTO.getSubject()+" </label>");
		}
	...
	//페이징
		...
		if (startBlock != 1) {
			...
			pageHtml.append("<a href='mailSend?cpage=");
		...
		for(int i=startBlock; i<=endBlock; i++) {
			if(i == cpage) {
				...
				pageHtml.append("mailSend?cpage=" + i);
				pageHtml.append("&recordPerPage=" + recordPerPage + "' ");
