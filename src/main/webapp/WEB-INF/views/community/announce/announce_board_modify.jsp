		...
		<script type="text/javascript" >
			window.onload = function() {
				...
				.create(document.querySelector('#editor'),{
						language: "ko",
						ckfinder: {
							uploadUrl : '/upload/announceboard'
						}
					})
					.then(editor => {
						  		...
						  		success: function(data) {
						  			if(data == 1) {
							  			Swal.fire({
								  			...
								  			willClose: () => {
								  				document.location.href='announceBoardView?cpage=<%=cpage%>&seq=<%=boardSeq%>';
							  				}
						  				})
						  			} else {
							  			Swal.fire({
		...
		<header class="top-margin py-5 backg-secondary">
			<div class="container px-4 px-lg-5 my-5">
				<div class="text-center text-white">
					...
					<p class="lead fw-normal text-white-50 mb-0">Announcement Modify</p>
				</div>
			</div>
		</header>
		<main>
			...
			<div class="container-fluid my-4 bottombody">	
 				<form action="/write/new" method="POST" enctype="multipart/form-data">
					...
					<div class="btn_area">			
						...
						<button type="button" class="btn btn-dark" onclick="location.href='announceBoardView?cpage=<%= cpage%>&seq=<%=boardSeq %>'" >보기</button>
